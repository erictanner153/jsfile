...
// ------------------------
// Watch for _URL links and replace them dynamically
// ------------------------
async function watchForLocalizedLinks() {
    const intervalId = setInterval(async () => {
        const localizedLinks = findElementsInShadowRoots('a[href$="_URL"]');

        if (localizedLinks.length > 0) {
            console.log("Found _URL links, replacing...");
            await replaceLocalizedLinks();
        } else {
            console.log("No more _URL links, stopping interval.");
    ...
    if(localizedLinks.length > 0){
        await Promise.all(
            localizedLinks.map(async (link) => {
                ...
                const contextAndKeyParts = href?.split('.');
                if (!contextAndKeyParts || contextAndKeyParts.length !== 2) return;

                await FluidTopicsCustomI18nService.prepareContext(contextAndKeyParts[0], {});
                const url = FluidTopicsCustomI18nService.resolveMessage(contextAndKeyParts[0], contextAndKeyParts[1]);
                if (url) {
                    link.href = url;
                }
            })
...
function runRedirections(useRouterService = false) {
    let docsLink = {
        ...
        "ie-quick-setup-v1.1/ie-hub-credentials": "access?ft:clusterId=/02-setting-up/02-setting-up/_entering-your-ie-hub-credentials",
        "ie-quick-setup-v1.1/validate": "access?ft:clusterId=/02-setting-up/02-setting-up/_validating-the-outcome",
