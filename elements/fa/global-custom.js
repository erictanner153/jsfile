...
function findElementsInShadowRoots(querySelector) {
    ...
    function searchInNode(node) {
        ...
        // search this root
        try {
            if (node.querySelectorAll) foundElements.push(...node.querySelectorAll(querySelector));
        } catch (_) {}
...
async function addElementToHeaderBar(element) {
    ...
    if (element.id && findElementsInShadowRoots("#" + element.id)?.[0]) return;
...
function updateBreadcrumbs(event) {
    ...
    if (isPreviewFromDifferentBook) return;

    let breadcrumbs = [];
    if (g_lastVisitedNonReaderPage) breadcrumbs.push(g_lastVisitedNonReaderPage);

    let docTitle = event.detail[0].parameters?.document?.title;
...
function setBreadcumbs(breadcrumbs) {
    ...
    for (const breadcrumb of breadcrumbs) {
        ...
        if (breadcrumbsContainer.children.length == 0) breadcrumbElement.classList.add("non-reader-page");
        else breadcrumbsContainer.appendChild(breadcrumbSeparator.cloneNode(true));
...
async function addReaderContentContainerResizeHandle() {
    ...
    readerContentContainers.forEach((readerContentContainer) => {
        ...
        readerContentContainerResizer.addEventListener("mouseenter", function () {
            readerContentContainerResizer.style.backgroundColor = "black";
        });
        readerContentContainerResizer.addEventListener("mouseleave", function () {
        ...
        readerContentContainerResizer.addEventListener("mousedown", function (e) {
            ...
            function onMouseMove(e) {
                ...
                if (newWidth >= minWidth && newWidth <= maxWidth) readerContentContainer.style.width = newWidth + "px";
            }
...
///////////////////////////////////////////////////////////////////////////////
// Industrial Edge (highlight.js left as-is; still disabled)
// initHighlightJS();

///////////////////////////////////////////////////////////////////////////////
// Mermaid + PlantUML (unchanged)
...
function runRedirections(useRouterService = false) {
    let docsLink = {
        ...
        "ie-quick-setup-v1.1/ie-hub-credentials":
            "access?ft:clusterId=/02-setting-up/02-setting-up/_entering-your-ie-hub-credentials",
        "ie-quick-setup-v1.1/validate":
            "access?ft:clusterId=/02-setting-up/02-setting-up/_validating-the-outcome",
    ...
    if (useRouterService) FluidTopicsRouterService.navigateTo(`/${docsLink[splitedPathname[1]]}`);
    else window.location.href = `${FTAPI.tenantBaseUrl}${docsLink[splitedPathname[1]]}`;
}
